 ...
 * 14-08-2013: Improved handling of read-modify-write cycles
 *
 *
 * Known Problems:
...
#define MMU030_DEBUG            1   /* enable(1) or disable(0) MMU general debugging messages */
#define MMU030_DEBUG_ERROR      1   /* enable(1) or disable(0) MMU error debugging messages */
#define MMU030_LOG_MEM_ACCESS   0   /* enable(1) or disable(0) ATC memory access logging */


#if MMU030_DEBUG
#define debug_msg   write_log
#else
#define debug_msg(...)
#endif

#if MMU030_DEBUG_ERROR
#define error_msg   write_log
#else
#define error_msg(...)
#endif

char table_letter[4] = {'A','B','C','D'}; /* only used for debugging messages */


/* Structs that hold all informations needed for address translation */

/* ATC struct for 68030 */
...
void mmu_op30_ptest (uaecptr pc, uae_u32 opcode, uae_u16 next, uaecptr extra)
{
     ...
     */
    if (!level && a) {
        error_msg("PTEST: Bad instruction causing F-line unimplemented instruction exception!\n");
        Exception(11, m68k_getpc(), M68000_EXC_SRC_CPU); /* F-line unimplemented instruction exception */
        return;
    }
        
    debug_msg("PTEST%c: addr = %08X, fc = %i, level = %i, ",
              rw?'R':'W', extra, fc, level);
    if (a) {
        debug_msg("return descriptor to register A%i\n", areg);
    } else {
        debug_msg("do not return descriptor\n");
    }
    ...
    debug_msg("PTEST status: %04X, B = %i, L = %i, S = %i, W = %i, I = %i, M = %i, T = %i, N = %i\n",
...
void mmu_op30_pload (uaecptr pc, uae_u32 opcode, uae_u16 next, uaecptr extra)
{
    ...
    debug_msg("PLOAD%c: Create ATC entry for %08X, FC = %i\n", write?'W':'R', extra, fc);
...
void mmu_op30_pflush (uaecptr pc, uae_u32 opcode, uae_u16 next, uaecptr extra)
{
    ...
    switch (mode) {
        case 0x1:
            debug_msg("PFLUSH: Flush all entries\n");
            mmu030_flush_atc_all();
            break;
        case 0x4:
            debug_msg("PFLUSH: Flush by function code only\n");
            debug_msg("PFLUSH: function code: base = %08X, mask = %08X\n", fc_base, fc_mask);
            mmu030_flush_atc_fc(fc_base, fc_mask);
            break;
        case 0x6:
            debug_msg("PFLUSH: Flush by function code and effective address\n");
            debug_msg("PFLUSH: function code: base = %08X, mask = %08X\n", fc_base, fc_mask);
            debug_msg("PFLUSH: effective address = %08X\n", extra);
            mmu030_flush_atc_page_fc(extra, fc_base, fc_mask);
            break;
            
        default:
            error_msg("PFLUSH ERROR: bad mode! (%i)\n",mode);
            break;
    }
}
...
uae_u32 mmu_op30_helper_get_fc(uae_u16 next) {
    switch (next&0x0018) {
        ...
        default:
            error_msg("MMU_OP30 ERROR: bad fc source! (%04X)\n",next&0x0018);
            return 0;
    }
}
...
void mmu030_flush_atc_page_fc(uaecptr logical_addr, uae_u32 fc_base, uae_u32 fc_mask) {
    ...
    for (i=0; i<ATC030_NUM_ENTRIES; i++) {
        if (((fc_base&fc_mask)==(mmu030.atc[i].logical.fc&fc_mask)) &&
            (mmu030.atc[i].logical.addr == (logical_addr&(~mmu030.translation.page.mask))) &&
            mmu030.atc[i].logical.valid) {
            mmu030.atc[i].logical.valid = false;
            debug_msg("ATC: Flushing %08X\n", mmu030.atc[i].physical.addr);
        }
...
void mmu030_flush_atc_page(uaecptr logical_addr) {
    int i;
    for (i=0; i<ATC030_NUM_ENTRIES; i++) {
        if ((mmu030.atc[i].logical.addr == (logical_addr&(~mmu030.translation.page.mask))) &&
            mmu030.atc[i].logical.valid) {
            mmu030.atc[i].logical.valid = false;
            debug_msg("ATC: Flushing %08X\n", mmu030.atc[i].physical.addr);
        }
...
void mmu030_flush_atc_all(void) {
    debug_msg("ATC: Flushing all entries\n");
    int i;
    for (i=0; i<ATC030_NUM_ENTRIES; i++) {
        mmu030.atc[i].logical.valid = false;
    }
...
int mmu030_match_ttr(uaecptr addr, uae_u32 fc, bool rmw, bool write)
{
    int tt0, tt1;

    bool cache_inhibit = false; /* TODO: pass to memory access function */
    
    tt0 = mmu030_do_match_ttr(tt0_030, mmu030.transparent.tt0, addr, fc, rmw, write);
    if (tt0&TT_OK_MATCH) {
        cache_inhibit = (tt0_030&TT_CI) ? true : false;
    }
    tt1 = mmu030_do_match_ttr(tt1_030, mmu030.transparent.tt1, addr, fc, rmw, write);
    if (tt1&TT_OK_MATCH) {
        if (!cache_inhibit) {
...
int mmu030_do_match_ttr(uae_u32 tt, TT_info comp, uaecptr addr, uae_u32 fc, bool rmw, bool write)
{
...
void mmu030_decode_tc(uae_u32 TC) {
    ...
    if (mmu030.translation.page.size<8) {
        error_msg("MMU Configuration Exception: Bad value in TC register! (bad page size: %i byte)\n",
    ...
    if ((shift-mmu030.translation.page.size)!=0) {
        error_msg("MMU Configuration Exception: Bad value in TC register! (bad sum)\n");
        Exception(56, m68k_getpc(), M68000_EXC_SRC_CPU); /* MMU Configuration Exception */
        return;
    }
     ...
     * and cause an exception (see above). */
    if (!TI_bits[0]) {
...
#define RP_ZERO_BITS    (UVAL64(0x0000FFFC00000000))
...
void mmu030_decode_rp(uae_u64 RP) {
    ...
    if (!descriptor_type) { /* If descriptor type is invalid */
        error_msg("MMU Configuration Exception: Root Pointer is invalid!\n");
        Exception(56, m68k_getpc(), M68000_EXC_SRC_CPU); /* MMU Configuration Exception */
    }
...
uae_u32 mmu030_table_search(uaecptr addr, uae_u32 fc, bool write, int level) {
    ...
    TRY(prb) {
        ...
        if ((tc_030&TC_ENABLE_SUPERVISOR) && super) {
            ...
            debug_msg("CPU Root Pointer: %08X%08X\n",descr[0],descr[1]);
        }
        ...
        switch (descr_type) {
            case DESCR_TYPE_INVALID:
                error_msg("Fatal error: Root pointer is invalid descriptor!\n");
                mmu030.status |= MMUSR_INVALID;
                goto stop_search;
            case DESCR_TYPE_EARLY_TERM:
                debug_msg("Root pointer is early termination page descriptor.\n");
        ...
        if (tc_030&TC_ENABLE_FCL) {
            debug_msg("Function code lookup enabled, FC = %i\n", fc);
            ...
            debug_msg("Table FCL at %08X: index = %i, ",table_addr,table_index);
            ...
            if (next_size==4) {
                ...
                debug_msg("Next descriptor: %08X\n",descr[0]);
            } else {
                descr[0] = phys_get_long(descr_addr[descr_num]);
                descr[1] = phys_get_long(descr_addr[descr_num]+4);
                debug_msg("Next descriptor: %08X%08X\n",descr[0],descr[1]);
            }
            ...
            switch (descr_type) {
                case DESCR_TYPE_INVALID:
                    debug_msg("Invalid descriptor!\n");
                    /* stop table walk */
                    mmu030.status |= MMUSR_INVALID;
                    goto stop_search;
                case DESCR_TYPE_EARLY_TERM:
                    debug_msg("Early termination page descriptor!\n");
                    early_termination = true;
                    goto handle_page_descriptor;
                case DESCR_TYPE_VALID4:
                    next_size = 4;
        ...
        do {
            ...
            debug_msg("Table %c at %08X: index = %i, ",table_letter[t],table_addr,table_index);
            ...
            if (descr_size==8) {
                ...
                if ((descr[0]&DESCR_LOWER_MASK) && (table_index<limit)) {
                    ...
                    debug_msg("limit violation (lower limit %i)\n",limit);
                    goto stop_search;
                }
                if (!(descr[0]&DESCR_LOWER_MASK) && (table_index>limit)) {
                    mmu030.status |= (MMUSR_LIMIT_VIOLATION|MMUSR_INVALID);
                    debug_msg("limit violation (upper limit %i)\n",limit);
                    goto stop_search;
                }
            }
        ...
        debug_msg("Page indirect descriptor at %08X: ",indirect_addr);
        ...
        if (next_size==4) {
            ...
            descr[0] = phys_get_long(descr_addr[descr_num]);
            descr[1] = phys_get_long(descr_addr[descr_num]+4);
            debug_msg("descr = %08X%08X",descr[0],descr[1]);
        }
    ...
    handle_page_descriptor:
        ...
        if (early_termination) {
            ...
            debug_msg("Logical address unused bits: %08X (mask = %08X)\n",
    ...
    stop_search:
        ...
        debug_msg("MMU: Bus error while %s descriptor!\n",
    ...
    if (i == ATC030_NUM_ENTRIES) {
        ...
        debug_msg("ATC is full. Replacing entry %i\n", i);
    }
    ...
    debug_msg("ATC create entry(%i): logical = %08X, physical = %08X, FC = %i\n", i,
              mmu030.atc[i].logical.addr, mmu030.atc[i].physical.addr,
              mmu030.atc[i].logical.fc);
    debug_msg("ATC create entry(%i): B = %i, CI = %i, WP = %i, M = %i\n", i,
...
void mmu030_ptest_atc_search(uaecptr logical_addr, uae_u32 fc, bool write) {
    ...
    if (mmu030_match_ttr(logical_addr, fc, false, write)&TT_OK_MATCH) {
 ...
 * least recently used entries. Entries are replaced if the cache is full.
...
void mmu030_page_fault(uaecptr addr, uae_u32 fc, bool rmw, bool read, int size) {    
    ...
    switch (size) {
        case sz_long: regs.mmu_ssw |= MMU030_SSW_SIZE_L; break;
        case sz_word: regs.mmu_ssw |= MMU030_SSW_SIZE_W; break;
        case sz_byte: regs.mmu_ssw |= MMU030_SSW_SIZE_B; break;
        default: break;
    }
...
void mmu030_put_long_atc(uaecptr addr, uae_u32 val, uae_u32 fc, bool rmw, int size, int l) {
    ...
    if (mmu030.atc[l].physical.bus_error || mmu030.atc[l].physical.write_protect) {
        mmu030_page_fault(addr, fc, rmw, false, size);
        return;
    }
...
void mmu030_put_word_atc(uaecptr addr, uae_u16 val, uae_u32 fc, bool rmw, int size, int l) {
    uae_u32 page_index = addr & mmu030.translation.page.mask;
    uae_u32 addr_mask = ~mmu030.translation.page.mask;
...
void mmu030_put_byte_atc(uaecptr addr, uae_u8 val, uae_u32 fc, bool rmw, int size, int l) {
    uae_u32 page_index = addr & mmu030.translation.page.mask;
    uae_u32 addr_mask = ~mmu030.translation.page.mask;
...
uae_u32 mmu030_get_long_atc(uaecptr addr, uae_u32 fc, bool rmw, int size, int l) {
    uae_u32 page_index = addr & mmu030.translation.page.mask;
    uae_u32 addr_mask = ~mmu030.translation.page.mask;
    ...
    if (mmu030.atc[l].physical.bus_error || (rmw && mmu030.atc[l].physical.write_protect)) {
        mmu030_page_fault(addr, fc, rmw, true, size);
        return 0;
    }
...
uae_u16 mmu030_get_word_atc(uaecptr addr, uae_u32 fc, bool rmw, int size, int l) {
    uae_u32 page_index = addr & mmu030.translation.page.mask;
    uae_u32 addr_mask = ~mmu030.translation.page.mask;
...
uae_u8 mmu030_get_byte_atc(uaecptr addr, uae_u32 fc, bool rmw, int size, int l) {
    uae_u32 page_index = addr & mmu030.translation.page.mask;
    uae_u32 addr_mask = ~mmu030.translation.page.mask;
...
int mmu030_atc_search(uaecptr addr, uae_u32 fc, bool write) {
...
void mmu030_atc_handle_history_bit(int entry_num) {
    ...
    if (j==ATC030_NUM_ENTRIES) {
        ...
        mmu030.atc[entry_num].mru = 1;
        debug_msg("ATC: No more history zero-bits. Reset all.\n");
    }
}
...
void mmu030_put_long(uaecptr addr, uae_u32 val, uae_u32 fc, bool rmw, int size) {
    
	//                                        addr,super,write
	if ((!mmu030.enabled) || (mmu030_match_ttr(addr,fc,rmw,true)&TT_OK_MATCH) || (fc==7)) {
		phys_put_long(addr,val);
		return;
    }

    int atc_line_num = mmu030_atc_search(addr, fc, true);

    if (atc_line_num<ATC030_NUM_ENTRIES) {
        mmu030_put_long_atc(addr, val, fc, rmw, size, atc_line_num);
    } else {
        mmu030_table_search(addr,fc,true,0);
        mmu030_put_long_atc(addr, val, fc, rmw, size, mmu030_atc_search(addr,fc,true));
    }
}

void mmu030_put_word(uaecptr addr, uae_u16 val, uae_u32 fc, bool rmw, int size) {
    ...
    if (atc_line_num<ATC030_NUM_ENTRIES) {
        mmu030_put_word_atc(addr, val, fc, rmw, size, atc_line_num);
    } else {
        mmu030_table_search(addr, fc, true, 0);
        mmu030_put_word_atc(addr, val, fc, rmw, size, mmu030_atc_search(addr,fc,true));
    }
}

void mmu030_put_byte(uaecptr addr, uae_u8 val, uae_u32 fc, bool rmw, int size) {
    ...
    if (atc_line_num<ATC030_NUM_ENTRIES) {
        mmu030_put_byte_atc(addr, val, fc, rmw, size, atc_line_num);
    } else {
        mmu030_table_search(addr, fc, true, 0);
        mmu030_put_byte_atc(addr, val, fc, rmw, size, mmu030_atc_search(addr,fc,true));
    }
}

uae_u32 mmu030_get_long(uaecptr addr, uae_u32 fc, bool rmw, int size) {
    
	//                                        addr,super,write
	if ((!mmu030.enabled) || (mmu030_match_ttr(addr,fc,rmw,false)&TT_OK_MATCH) || (fc==7)) {
		return phys_get_long(addr);
    }
    
    int atc_line_num = mmu030_atc_search(addr, fc, false);

    if (atc_line_num<ATC030_NUM_ENTRIES) {
        return mmu030_get_long_atc(addr, fc, rmw, size, atc_line_num);
    } else {
        mmu030_table_search(addr, fc, false, 0);
        return mmu030_get_long_atc(addr, fc, rmw, size, mmu030_atc_search(addr,fc,false));
    }
}

uae_u16 mmu030_get_word(uaecptr addr, uae_u32 fc, bool rmw, int size) {
    ...
    if (atc_line_num<ATC030_NUM_ENTRIES) {
        return mmu030_get_word_atc(addr, fc, rmw, size, atc_line_num);
    } else {
        mmu030_table_search(addr, fc, false, 0);
        return mmu030_get_word_atc(addr, fc, rmw, size, mmu030_atc_search(addr,fc,false));
    }
}

uae_u8 mmu030_get_byte(uaecptr addr, uae_u32 fc, bool rmw, int size) {
    ...
    if (atc_line_num<ATC030_NUM_ENTRIES) {
        return mmu030_get_byte_atc(addr, fc, rmw, size, atc_line_num);
    } else {
        mmu030_table_search(addr, fc, false, 0);
        return mmu030_get_byte_atc(addr, fc, rmw, size, mmu030_atc_search(addr,fc,false));
    }
}


uae_u16 REGPARAM2 mmu030_get_word_unaligned(uaecptr addr, uae_u32 fc, bool rmw)
{
	uae_u16 res;
    
	res = (uae_u16)mmu030_get_byte(addr, fc, rmw, sz_word) << 8;
	SAVE_EXCEPTION;
	TRY(prb) {
		res |= mmu030_get_byte(addr + 1, fc, rmw, sz_word);
		RESTORE_EXCEPTION;
	}
	CATCH(prb) {
		RESTORE_EXCEPTION;
...
uae_u32 REGPARAM2 mmu030_get_long_unaligned(uaecptr addr, uae_u32 fc, bool rmw)
{
	uae_u32 res;
    
	if (likely(!(addr & 1))) {
		res = (uae_u32)mmu030_get_word(addr, fc, rmw, sz_long) << 16;
		SAVE_EXCEPTION;
		TRY(prb) {
			res |= mmu030_get_word(addr + 2, fc, rmw, sz_long);
			RESTORE_EXCEPTION;
		}
		CATCH(prb) {
			RESTORE_EXCEPTION;
//			regs.mmu_fault_addr = addr;
		...
		res = (uae_u32)mmu030_get_byte(addr, fc, rmw, sz_long) << 8;
		SAVE_EXCEPTION;
		TRY(prb) {
			res = (res | mmu030_get_byte(addr + 1, fc, rmw, sz_long)) << 8;
			res = (res | mmu030_get_byte(addr + 2, fc, rmw, sz_long)) << 8;
			res |= mmu030_get_byte(addr + 3, fc, rmw, sz_long);
			RESTORE_EXCEPTION;
		}
		CATCH(prb) {
			RESTORE_EXCEPTION;
...
void REGPARAM2 mmu030_put_long_unaligned(uaecptr addr, uae_u32 val, uae_u32 fc, bool rmw)
{
	SAVE_EXCEPTION;
	TRY(prb) {
		if (likely(!(addr & 1))) {
			mmu030_put_word(addr, val >> 16, fc, rmw, sz_long);
			mmu030_put_word(addr + 2, val, fc, rmw, sz_long);
		} else {
			mmu030_put_byte(addr, val >> 24, fc, rmw, sz_long);
			mmu030_put_byte(addr + 1, val >> 16, fc, rmw, sz_long);
			mmu030_put_byte(addr + 2, val >> 8, fc, rmw, sz_long);
			mmu030_put_byte(addr + 3, val, fc, rmw, sz_long);
		}
		RESTORE_EXCEPTION;
	}
	CATCH(prb) {
...
void REGPARAM2 mmu030_put_word_unaligned(uaecptr addr, uae_u16 val, uae_u32 fc, bool rmw)
{
	SAVE_EXCEPTION;
	TRY(prb) {
		mmu030_put_byte(addr, val >> 8, fc, rmw, sz_word);
		mmu030_put_byte(addr + 1, val, fc, rmw, sz_word);
		RESTORE_EXCEPTION;
	}
	CATCH(prb) {
		RESTORE_EXCEPTION;
